...
<body>
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped table-hover">
                      <thead>
                        ...
                        $sql = "SELECT * FROM opr_invoice WHERE invoice_id != 'Waiting'";
              ...
              <tr>
                ...
                <th>Action</th>
                          ...
                          while ($row = $result->fetch_assoc()) {
                            ...
                            $percentagePaid = 100; // Default to 0 to avoid division by zero
                            $pending = $row['totalFee'] - $row['totalPaid'];
                            if ($pending > 0) {
                              $percentagePaid = ($row['totalPaid'] / $row['totalFee']) * 100;
                            }
                            echo '<tr>
                    ...
                    <td>' . number_format($row['totalFee'], 3) . '</td>
                    <td id="paidCell_"' . $row['id'] . '>' . number_format($row['totalPaid'], 3) . '</td>
                    ...
                    <button type="button" class="btn btn-primary btn-icon-text" onclick="lock(' . $row['id'] . ')">
  ...
  <script>
    function openInvoicePage(id) {
      window.open('../../pages/invoice/invoice.php?id=' + id, '_blank');
    }
    ...
    function updateDeposit() {
      ...
      fetch('../../pages/invoice/deposit.php', {
          method: 'POST',
          headers: {
