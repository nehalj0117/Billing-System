...
<body>
        ...
        <li class="nav-item">
          ...
          <div class="collapse" id="aircraft-menu">
            <ul class="nav flex-column sub-menu">
              ...
              <li class="nav-item">
                <a class="nav-link" href="../../../pages/aircraft/add-edit-invoice-comments.php">Edit Invoice
                  Comments</a>
              </li>
              <li class="nav-item">
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped table-hover">
                      <thead>
                        ...
                        $sql = "SELECT ri.id, ri.invoiceId, ri.exemption, ri.overDue, ri.invoiceDate, feeDescription, ri.paid, ri.dueDate, ri.occurenceType, ri.customerName, ri.feeCode, ri.feeName, ri.contractNo, ri.contractName, ri.contractStartDate, ri.contractEndDate, ri.amount, ri.charged
                        ...
                        echo '<th>Progress</th>';
                        echo '<th>Actions</th>';
                        echo '<th hidden>Fee Description</th>';
                        echo '</tr>';
                        echo '</thead>';
                        echo '<tbody>';
                        if ($result->num_rows > 0) {
                          while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>" . $row["feeCode"] . "</td>";
                            echo "<td>" . $row["feeName"] . "</td>";
                            ...
                            echo "<td>
                                  <button type='button' class='btn btn-success btn-icon-text' onclick='openInvoicePage(" . $row['id'] . ")'>
                                  <i class='mdi mdi-check btn-icon-prepend'></i> Print
                                  </button>
                                  <button type='button' class='btn btn-warning btn-icon-text' onclick='deposit(" . $row['id'] . ")'>
                                  <i class='mdi mdi-cash-multiple btn-icon-prepend'></i> Deposit
                                  </button>
                                  <button type='button' class='btn btn-primary btn-icon-text' onclick='complete(" . $row['id'] . ")'>
                                  <i class='mdi mdi-file-lock btn-icon-prepend'></i> Lock
                                  </button>
                                  </td>";
                            echo "<td hidden>" . $row["feeDescription"] . "</td>";
                          }
                        } else {
                    ...
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="depositModalLabel">Enter Deposit Amount</h5>
                        ...
                        <div class="modal-body">
                          ...
                          <input type="hidden" id="depositRowId">
                        </div>
                        <div class="modal-footer">
                          <button onclick="updateDeposit()" class="btn btn-success">Confirm</button>
  ...
  <script>
    ...
    function deposit(rowId) {
      ...
      $('#depositModal').modal('show'); // Using jQuery to show the modal
    }

    function closeModal() {
      $('#depositModal').modal('hide'); // Using jQuery to hide the modal
    }
