...
<body>
      ...
      <div class="main-panel">
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped table-hover">
                      <thead>
                        ...
                        $sql = "SELECT ri.id, ri.customerName, ri.overDue, ri.notice1, ri.notice2, ri.notice3, ri.invoicePeriod, ri.invoiceId, ri.exemption, ri.invoiceDate, ri.paid, ri.dueDate, ri.occurenceType, ri.customerName, ri.feeCode, ri.feeName, ri.contractNo, ri.contractName, ri.contractStartDate, ri.contractEndDate, ri.amount, ri.charged
                            FROM rental_invoice ri
                            WHERE ri.invoiceDate BETWEEN '$fromDate' AND '$toDate'
                            ORDER BY ri.id ASC";
                        ...
                        if ($result->num_rows > 0) {
                          while ($row = $result->fetch_assoc()) {
                            ...
                            if ($row['paid'] > 0) {
                              $percentagePaid = ($row['paid'] / $row['amount']) * 100;
                            }
                            if ($row['overDue'] != 0) {
                              $invoicefee = ($row['overDue'] + $row["amount"]);
                            }
                            if ($row['exemption'] != 0.00) {
                              $invoicefee = $invoicefee - (($row['exemption'] / 100) * $invoicefee);
                            }
                            $disableApprove = "";
                            $buttonStyle = "btn-success";
                            if ($row['invoiceId'] != "Waiting") {
                            ...
                            echo "<td>
                                    ...
                                    <button type='button' class='btn btn-info btn-icon-text' onclick='editRow(" . $row['id'] . ")'>
                                      <i class='mdi mdi-cash-multiple btn-icon-prepend'></i> Edit
                                    </button>
                                    <button type='button'" . $disableApprove . " class='btn ".$buttonStyle." btn-icon-text' onclick='approveRow(" . $row['id'] . ")'>
        ...
        <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">Edit OverDue Value</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <label for="overDueValue">OverDue Value:</label>
                <input type="number" id="overDueValue" class="form-control">
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
  ...
  <script>
    ...
    document.getElementById('confirmEdit').addEventListener('click', function () {
      overDueValue = document.getElementById('overDueValue').value;
      console.log(overDueValue);
      $.ajax({
        type: "POST",
        url: "edit_overDue.php",
        data: { id: selectedRowId, overDue: overDueValue },
        success: function (response) {
          alert(response);
          location.reload();
        },
        error: function () {
          alert("An error occurred while editing the over due value.");
        }
      });
